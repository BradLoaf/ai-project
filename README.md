...
# Running an Agent
In the main branch there are 2 options for agents to run. Base PPO and PPO with a GNN feature extractor. To run an agent run the command `python3 run_agent.py {model_folder}` where `model_folder` is the folder containing the model of your choice. There is also the option to include the deterministic flag before running with `-d` or `--deterministic`. This will cause the model to only select the action with the highest probability rather than sample from a distribution.
